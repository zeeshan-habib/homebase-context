...
| **Hourly Wages** | Average of per-job wage rates (`total_wages_earned / hours_worked`) across all jobs in the payroll cohort, for the reference month | `corona.shift_and_timecard_events` (filtered to payroll cohort from `postgres.payroll_payroll_runs`) | `wage_rate` per `job_id` = `SUM(total_wages_earned) / SUM(hours_worked)`; then `AVG(wage_rate)` across jobs | Overall; by industry (`locations.business_type`); by state; by MSA |
...
| Column | Definition |
|---|---|
| `engagement_boolean` | `1` if the location is engaged: (a) a shift was created, (b) ≥20% of employees at the location created a timecard, AND (c) at least one owner activity in the past 30 days. Measured over a 7-day window. |
| `scheduling_engaged_boolean` | Same as `engagement_boolean` but scoped to the scheduling product specifically. |
...
| Column | Definition |
|---|---|
| `two_d_thirty_active_this_month` | `1` if the location had any employee activity on ≥2 days within the past 30 days. This is the company-wide **2D30 active** definition — a loose but widely-used activity threshold. More inclusive than `engagement_boolean`. |
...
Same cohort and `timeseries_data` CTEs as above. In `job_averages`, add `location_id` to the GROUP BY. Then join to `public.locations` on `location_id` and group by `locations.business_type`. Filter `state NOT IN ('Not USA', 'Unclassified')`.
...
<!-- STUB: Add shifts worked query and additional metric-level aggregation queries once confirmed with Vlad. -->
