...
refs:
  ...
  - data/product-areas/mshr/workflows/known-pitfalls.md
---

# MSHR — Methodology Reference

Load when generating MSHR SQL or Python, or when determining which methodology applies to a given request type.

For known bugs and required code structure, see `known-pitfalls.md`. For production example queries (wages, hiring, turnover), see `../example-queries.md`.

---

## Request Type Classification

MSHR data requests fall into one of the following types. Each type has a canonical source table, aggregation approach, and qualification logic.

| If the request involves... | Request type |
|---|---|
...
City values in `dbt.temp_timeclock_data` are mixed case (`'Miami'`, `'MIAMI'`, `'miami'` all appear). MSA values have similar variance.
...
These datasets are 3–4 GB. Queries must follow these rules for acceptable Databricks performance:
...
## Normalization Rules for Ad Hoc Reports

Raw aggregate totals conflate platform growth (more locations joining Homebase over time) with real economic signal. Normalized per-location metrics are comparable across time and geography.
...
2. **Raw totals are context, not headlines.** Always compute and return the raw totals alongside the normalized figures so the reader can sanity-check — but label them clearly as supporting data, not the primary metric.
